...
import { wsConnectOrder, wsDisconnectOrder } from '../../services/reducers/orders/actions';

export const MyOrdersPage = () => {
    const dispatch = useDispatch()
    React.useEffect(() => {
        dispatch(wsConnectOrder({wsUrl: BURGER_API_WSS_ORDERS, withTokenRefresh: false}))
        return () => {
    ...
    // const legnth: any = data?.orders.length
    return (
            ...
            {1 > 0
                ? <ul className={`${styles.orderFeed} custom-scroll`}>
                    {data?.orders?.map((order: TOrder) => {
                        return <OrdersFeedCard order={[order._id]} key={order?._id} timeStamp={String(order.createdAt)} orderName={order.name} ingredients={order.ingredients} orderNumber={order.number}/>
                    })}
                </ul>
