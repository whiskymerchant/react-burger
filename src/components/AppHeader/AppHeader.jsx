  ...
  return (
    <header className={cn(styles.header, 'p-4')}>
      <nav className={cn(styles.constructor)}>
        ...
        <span className={cn('text text_type_main-default ml-2')}>Конструктор</span>
      </nav>
      <nav className={styles.feed}>
        <ListIcon type="secondary" /> 
        <span className={cn('text text_type_main-default text_color_inactive ml-2')}>Лента заказов</span>
      </nav>
      <nav className={styles.account}>
        <ProfileIcon type="secondary" />
        <span className={cn('text text_type_main-default text_color_inactive ml-2')}>Личный кабинет</span>
      </nav>
